      ...
      <div class="w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-6"
        ...
        <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
          ...
          <div class="space-y-1.5">
            ...
            <input formControlName="name" maxlength="255" placeholder="Nombre del workspace"
          ...
          <div class="flex gap-3 pt-1">
            ...
            <button type="submit" [disabled]="loading() || form.invalid"
              ...
              {{ loading() ? (workspace ? 'Guardando...' : 'Creando...') : (workspace ? 'Guardar cambios' : 'Crear workspace') }}
            </button>
          </div>
        </form>
      </div>
...
export class WorkspaceFormModalComponent implements OnInit {
  @Input() workspace: WorkspaceAccess | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() saved = new EventEmitter<WorkspaceOut>();
  ...
  submit(): void {
    ...
    const request$ = this.workspace
      ? this.workspaceService.update(this.workspace.id, name.trim(), description.trim() || null)
      : this.workspaceService.create(name.trim(), description.trim() || null);

    request$.subscribe({
      next: (ws) => { this.saved.emit(ws); this.close.emit(); },
